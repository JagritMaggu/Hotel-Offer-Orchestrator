...
exports.fetchFromSupplierA = fetchFromSupplierA;
exports.fetchFromSupplierB = fetchFromSupplierB;
exports.saveHotelsToRedis = saveHotelsToRedis;
const mockSuppliers_1 = require("../data/mockSuppliers");
const redis_1 = require("../redis");
async function fetchFromSupplierA(city) {
    console.log(`[Activity: fetchFromSupplierA] Fetching data for city: ${city}`);
    try {
        const response = await fetch(`http://localhost:3000/supplierA/hotels?city=${city}`);
        if (!response.ok) {
...
async function fetchFromSupplierB(city) {
    console.log(`[Activity: fetchFromSupplierB] Fetching data for city: ${city}`);
    try {
        const response = await fetch(`http://localhost:3000/supplierB/hotels?city=${city}`);
        if (!response.ok) {
...
async function saveHotelsToRedis(city, hotels) {
